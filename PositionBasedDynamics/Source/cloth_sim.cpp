...
{// update the system for a certain time step dt.
    ...
    apply_external_force(gravity, dt);//done
    damp_velocity(0.01f);
    compute_predicted_position(dt);//done
...
{// TODO: generate all internal constraints in this function.
    ...
    for(i = 0; i < m_dimx; ++i)
    {
        for(k = 0; k < m_dimz; ++k)
        {
			...
			if(k == 0)
			{
				FixedPointConstraint* fixedConstraint;
				fixedConstraint = new FixedPointConstraint(&m_vertices, index, m_vertices.pos(index));

				//m_constraints_int.push_back(fixedConstraint);
			}
        }
    }
    ...
    {// TODO: add stretch constraint here.
		unsigned int start = e->m_v1;
		unsigned int end = e->m_v2;
		
		float restLength = glm::length(m_vertices.pos(start) - m_vertices.pos(end));
        StretchConstraint* stretchConstraint;
		stretchConstraint = new StretchConstraint(&m_vertices, s_stiff, start, end, restLength);
    ...
    for(std::vector<Edge>::iterator e = m_edge_list.begin(); e != m_edge_list.end(); ++e)
    {
		...
		bendP1 = m_vertices.pos(id1);
		bendP2 = m_vertices.pos(id2);
		bendP3 = m_vertices.pos(id3);
		bendP4 = m_vertices.pos(id4);
		...
		vec3 newP1(0, 0, 0);
		vec3 newP2 = bendP2 - bendP1;
		vec3 newP3 = bendP3 - bendP1;	
		vec3 newP4 = bendP4 - bendP1;
		...
		bendConstraint = new BendConstraint(&m_vertices, b_stiff, id1, id2, id3, id4, theta);
		m_constraints_int.push_back(bendConstraint);

		//float bendRestLength = glm::length(m_vertices.pos(id3) - m_vertices.pos(id4));
		//StretchConstraint* stretchConstraint;
		//stretchConstraint = new StretchConstraint(&m_vertices, b_stiff, id3, id4, bendRestLength);
		//m_constraints_int.push_back(stretchConstraint);
    }
}
...
{// apply external force to cloth.
    ...
    {// TODO: apply force to velocity of all vertices.
		...
		m_vertices.vel(i) += dt * force * m_vertices.inv_mass(i);
    }
}
...
void ClothSim::damp_velocity(float k_damp)
{
	...
	for(int i = 0; i < m_vertices.size(); ++i)
	{
		...
		float mass = 1 / m_vertices.inv_mass(i);
		massPos += mass * pos;
		massVel += mass * vel;
		totalMass += mass;
	}
	...
	for(int i = 0; i < m_vertices.size(); ++i)
	{
		float mass = 1 / m_vertices.inv_mass(i);
		glm::vec3 vel = m_vertices.vel(i);
		glm::vec3 r = m_vertices.pos(i) - centerMassPos;
	...
	for(int i = 0; i < m_vertices.size(); ++i)
	{
		...
		m_vertices.vel(i) += k_damp * deltaV;
	}
...
{// update velocity based on collision restitution or friction.
    ...
    {// TODO: modify the velocity according to collisions
		...
		vec3 currentVel = m_vertices.vel(pointIndex);
		normal = s->normal();
		if(glm::dot(normal, currentVel) < 0)
		{
			...
			vec3 newVn = -1 * restitution * vn;
			vec3 newVt = friction * vt;
		
			m_vertices.vel(pointIndex) = newVn + newVt;
		}
		int a = 0;
    }
